...
public:
  ...
  CPXDEPRECATED void writeVectors(const char* name) const;
  CPXDEPRECATED void writeTextSolution(const char* name) const;
  ...
  CPXDEPRECATED void readVectors(const char* name) const;
  ...
  public:
    ...
    void setParam(StringParam  which, char* val) {
       IloAssert(_impl, "Using empty handle IloCplex::ParameterSet");
       _impl->setParam(which, val);
    }
...
#define CPX_MIPSEARCH_DYNAMIC            2
  ...
  void getIIS(IISStatusArray cstat, IloNumVarArray var,
  ...
  typedef int* Index;
  typedef IloArray<Index> IndexArray;
  ...
  class SolveCallbackI : public ControlCallbackI {
    Callback::Type getType() const;
    void doSetVectors(const IloNumArray    x,
                      const IloNumVarArray var,
                      const IloNumArray    pi,
                      const IloRangeArray  rng);
    void doSetVectors(const IloNumArray    x,
  ...
  public:
    ...
    IloNum evaluateNode(CpxNodeI *node, IloInt nIndex);
...
public:
    inline Parameter(IloInt key, CPXINT value, bool isBool = 0);
    inline Parameter(IloInt key, CPXLONG value);
    inline Parameter(IloInt key, IloNum value);
    inline Parameter(IloInt key, char* value);
    inline Type getType() const;
    inline IloInt getKey() const;
...
inline IloCplex::Parameter::Parameter(IloInt key, CPXINT value, bool isBool) :
...
inline IloCplex::Parameter::Parameter(IloInt key, CPXLONG value) :
    _t(Long), _key(key) {
    _value.Long = value;
}

inline IloCplex::Parameter::Parameter(IloInt key, IloNum value) :
    _t(Num), _key(key) {
    _value.Num = value;
}

inline IloCplex::Parameter::Parameter(IloInt key, char* value) :
    _t(String), _key(key) {
    _value.String = value;
}
...
inline IloInt IloCplex::Parameter::getKey() const {
    return _key;
}
...
IloCplex::ParameterSet::Iterator::operator*() const {
    switch (state) {
        ...
        case Parameter::Bool:
            return Parameter(_ps->_boolParams[pos], _ps->_boolParamValues[pos],
        		     1);
...
const char* CPXPUBLIC rowName (void* data, CPXLONG ij, char* nameBuf);
const char* CPXPUBLIC colName (void* data, CPXLONG ij, char* nameBuf);
const char* CPXPUBLIC quadName(void* data, CPXLONG ij, char* nameBuf);
const char* CPXPUBLIC sosName (void* data, CPXLONG ij, char* nameBuf);
}
...
private:
  ...
  char _tmpname[256];
  friend const char* CPXPUBLIC rowName (void* data, CPXLONG ij, char *nameBuf);
  friend const char* CPXPUBLIC colName (void* data, CPXLONG ij, char *nameBuf);
  friend const char* CPXPUBLIC quadName(void* data, CPXLONG ij, char *nameBuf);
  friend const char* CPXPUBLIC sosName (void* data, CPXLONG ij, char *nameBuf);
...
private:
  ...
  IloCachedArray<char> _sostype;
  IloSVectorArray*     _soscache;
  void flushSOS();
  void removeFromSOS(Index col);
  void removeFromSOS(const IndexArray cols, IloCarray<int>& mark);
  ...
  // result caches
  IloCplex::CplexStatus  _solstat;
  IloCachedArray<double> _qcslack;
  IloCachedArray<double> _slack;
  IloCachedArray<double> _x;
  IloCachedArray<double> _pi;
  IloCachedArray<double> _dj;
  IloCachedArray<IloCplex::BasisStatus> _rstat;
  IloCachedArray<IloCplex::BasisStatus> _cstat;
  IloCachedMultiArray<double> _solnpoolx;
  IloCachedMultiArray<double> _solnpoolslack;
  IloCachedMultiArray<double> _solnpoolqcslack;
  ...
  void    finitGoals(IloBool insubtree, IloBool mipstart);
  // END: For the Goal API

  IloBool doSolve(IloBool isLP=IloTrue);
...
public:
  ...
  Index getVarIndex (int id) const;
  ...
  Index getConIndex (int id) const;
  ...
  IloCarray<int> _conflictGroupId;
  ...
  int getNSOSs() const { return (_sostype.getSize()); }
  ...
  int isMIP() const;
  int isQC() const     { return (_nQCs > 0); }
  int isQO() const      { return (_hasQ); }
  ...
  void copyQ(const int* beg, const int* cnt,
  ...
  SOS getSOS(Index i);
  Index makeSOS(const IloExtractableI* obj, IloInt type);
...
protected:
  ...
  IloCplexColumn(IloCplexI* cplex, int j)
    : _cpxenv(cplex->_cpxenv), _lp(cplex->_lp), _cache(cplex->_cache), _col(j) {
  }

public:
  void add(IloNum val, IloInt row);
...
protected:
  ...
  IloCplexRow(IloCplexI* cplex, int j)
    : _cpxenv(cplex->_cpxenv), _lp(cplex->_lp), _cache(cplex->_cache), _row(j) {
  }

public:
  void add (IloNum val, IloInt col);
...
class IloCplexSOS {
  ...
  IloCplexSOS(IloCplexI* cplex, IloCplex::Index j);
  IloCplexSOS(IloCplexI* cplex, IloInt type);

public:
  IloCplexSOS(const IloCplexSOS& sos);
  ~IloCplexSOS();
  IloNum getMax() const;
  void add (IloNum val, IloCplex::Index col);
  void add (IloCplex::Index col, IloNum val) { add (val, col); }
};
...
inline IloCplexI::SOS
IloCplexI::getSOS(Index i) {
  return (IloCplexI::SOS(this, i));
}
