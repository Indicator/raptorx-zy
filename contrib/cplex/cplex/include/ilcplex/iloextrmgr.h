...
class IloLPExtractorManager : public IloPlugIn {
  ...
  IloExtractableArray _added;    // added during extraction
  IloExtractableArray _failed;   // extractables for which extraction failed

  struct ExList {
     IloCpxExtractor* _extr;    // extractor responsible for extraction
     Data*            _data;    // Data object describing extraction
     int              _usecnt;  // # of times object has been extracted.  If a
  ...
  IloArray<ExList> _which;

  IloExprExtractor*  _firstExpr;
  IloArray<ExList>   _whichExpr;
   ...
   * creates.
   */
  Data* makeData (void* data, int firstadded, int lastadded);
  ...
  void pushConData (IloInt id, int firstadded, int lastadded, void *data =0);
...
public:
  ...
  IloInt getConNadded(int id) const;
  ...
  IloExtractable getConAdded(int id, int n) const;
  IloExtractable getConAdded(const IloExtractableI* obj, int n) const {
     return (getConAdded(obj->getId(), n));
  }
  IloExtractable getConAdded(IloExtractable obj, int n) const {
     return (getConAdded(obj.getImpl(), n));
  }
  void setConAdded(const IloExtractableI* obj, int n, IloExtractable set);
  void setConAdded(IloExtractable obj, int n, IloExtractable set) {
     setConAdded(obj.getImpl(), n, set);
  }
  IloExtractable getConLastAdded(IloExtractable obj);

  IloCplexI::Index getConIndex (int id) const {
     return ((id >= 0 && id < _which.getSize() && _which[id]._data != 0)
             ? _which[id]._data->index : 0);
  }
  ...
  IloInt getVarNadded(int id) const;
  ...
  IloExtractable getVarAdded(int id, int n) const;
  IloExtractable getVarAdded(const IloExtractableI* obj, int n) const {
     return (getVarAdded(obj->getId(), n));
  }
  IloExtractable getVarAdded(IloExtractable obj, int n) const {
     return (getVarAdded(obj.getImpl(), n));
  }
  void setVarAdded(const IloExtractableI* obj, int n, IloExtractable set);
  void setVarAdded(IloExtractable obj, int n, IloExtractable set) {
     setVarAdded(obj.getImpl(), n, set);
  }
  IloExtractable getVarLastAdded(IloExtractable obj) const;

  IloCplexI::Index getVarIndex(int id) const {
     return (_whichExpr[id]._data != 0 ? _whichExpr[id]._data->index : 0);
  }
  ...
  IloCplexI::Index makeIndex(const IloNumExprI* expr) {
     int id = expr->getId();
     return (_whichExpr[id]._data != 0  &&  ++_whichExpr[id]._usecnt)
             ? _whichExpr[id]._data->index
             : doMakeIndex (expr);
  }
  IloCplexI::Index makeIndex(const IloNumExpr expr) {
   ...
   IloNumVar makeNumVar (const IloNumExprI* expr) {
      IloCplexI::Index index = makeIndex(expr);
      return (index
              ? IloNumVar((IloNumVarI*)_alg->getColumnExtractable(index))
              : IloNumVar(0)                                             );
   }
