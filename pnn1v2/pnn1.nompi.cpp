...
void Sequence::buildbatch(double p) {
    ...
    }
    {
        ...
        maxgot[m_pModel->num_labels-1] = nlabel[m_pModel->num_labels-1]*0.1;
        if (proc_id == 0)cerr << "\n";
        for (int j = 0; j < nDataPoints; j++) {
            int i = shff[j];
            //if(obs_label[i]==m_pModel->num_labels-1)
...
Score PairwiseSequence::Predict(_run_mode run_mode) {
    ...
    if (run_mode == _run_mode::training) {
        ...
        string fn = p_name.substr(0, p_name.length() - 1) + ".epad.prob";
        ...
        for (int i = 0; i < length_contact; i++) {
            ...
            for (int j = 2; j < m_pModel->num_labels; j++)
...
//
    int total_trainseq=m_pModel->num_data;
//
//
//
//
...
void Nnpf::SetParameters(int model, int w_size, int n_labels, int n_gates,
        int n_local, int n_pairwise,int pair_window, bool init_from_file) {
...
void Nnpf::Initialize(int model, string model_dir, int w_size, int n_labels, 
        int n_gates, string columns, string pairwise_columns, string input_f, 
        string split_file, int pair_windowsize,string init_file) {
    //pair_windowsize=w_size;
    char fname[100];
    h5dir = "";
    bool init_from_file;
    init_from_file=(init_file!="");
    if (init_from_file)
        ...
        if (init_from_file) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (fin.gcount() == 0)
                    ...
                    si >> tmp >> data;
                    if (tmp == "num_params:"){
                        num_params = data;weights = new double[num_params];
                    }
                    else if (tmp == "num_gates:")
                        n_gates = num_gates = data;
                    else if (tmp == "window_size:")
                        w_size= window_size = data;
                    else if (tmp == "dim_features:")
                        dim_features = data;
                    else if (tmp == "num_labels:")
                        n_labels = num_labels = data;
                    else if (tmp == "dim_one_pos:")
                        dim_one_pos = data;
                    else if (tmp == "pair_window_size:")
                        pair_windowsize = pair_window_size = data;
                    cerr << tmp << "::::" << data << endl;
                }
            }
    ...
    SetParameters(model, w_size, n_labels, n_gates, n_local, n_pairwise, pair_windowsize,init_from_file);
    trainData.clear();
    testData.clear();
        ...
        if (seq_used)
            try {
                ...
                if(run_mode==_run_mode::training)
                    ...
                    string pdbid=seq->p_name.substr(0,seq->p_name.length()-1);
                    
                    seq->Pnnloadmi(seq->p_name,pdbid+".h5");
                }
                /*
                ...
                for (int i = 0; i < seq->length_seq; i++) {
                    for (int j = i + 1; j < seq->length_seq; j++) {
                        vector<pair<int, int> > wi1 = find_feature_window(i, j, window_size, seq->length_seq);
                        int offset = 0;
                        int fp = 0;
                        ...
                        wi1 = find_feature_window_pair(i, j, pair_windowsize, seq->length_seq);
