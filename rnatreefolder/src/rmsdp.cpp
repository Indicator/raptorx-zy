...
/*========================================
 *
 *rmsd calculation for this problem only
 *
 *
 *
 *
...
void RMSD(double** ref_xlist,
               ...
               double* rmsd)
{
  if(n_list==0) { (*rmsd)=10000; return;}
//  double mov_com[3];
//  double mov_to_ref[3];
//  double U[3][3];
   calculate_rotation_rmsd(ref_xlist,mov_xlist, 
			     n_list,
			     mov_com,
			     mov_to_ref,
			     U,
...
//  double U[3][3];
   calculate_rotation_rmsd(ref_xlist,mov_xlist, 
			     ...
			     U,
			     rmsd);
   memcpy(U0,U,sizeof(double)*9);
   memcpy(mov_to_ref0,mov_to_ref,sizeof(double)*3);
}
...
//  double mov_to_ref[3];
//  double U[3][3];
  calculate_rotation_rmsdPoint(ref_xlist,mov_xlist, 
			     n_list,
			     mov_com,
			     mov_to_ref,
			     U,
			     rmsd, position, distance);
}
...
int calculate_rotation_matrixPoint(double** ref_xlist,
                             double** mov_xlist, double R[3][3],
                              double U[3][3], 
                              double E0,
                              double* residual, int position, double* distance)
{
  int i, j, k;
//  double Rt[3][3], RtR[3][3];
//  double left_eigenvec[3][3], right_eigenvec[3][3], eigenval[3];
//  double v[3];
  ...
  seqX = mov_xlist[position][0];
  seqY = mov_xlist[position][1];
  seqZ = mov_xlist[position][2];
  
  tempX = ref_xlist[position][0]*U[0][0]+ref_xlist[position][1]*U[1][0]+ref_xlist[position][2]*U[2][0];
  tempY = ref_xlist[position][0]*U[0][1]+ref_xlist[position][1]*U[1][1]+ref_xlist[position][2]*U[2][1];
  tempZ = ref_xlist[position][0]*U[0][2]+ref_xlist[position][1]*U[1][2]+ref_xlist[position][2]*U[2][2];
  
  double tempSum;
  tempSum = (seqX-tempX)*(seqX-tempX)+(seqY-tempY)*(seqY-tempY)+(seqZ-tempZ)*(seqZ-tempZ);
  
  *distance = sqrt(tempSum);
  //cout<<"$$$$$$$$$$$$$$$$$$$$$$$On position: "<<position<<", the distance is: "<<*distance<<endl;
  *residual = E0 - (double) sqrt(fabs(eigenval[0])) 
                 - (double) sqrt(fabs(eigenval[1]))
                 - sigma * (double) sqrt(fabs(eigenval[2]));
                             ...
                             double* rmsd, int position, double* distance)
{
  ...
  setup_rotation(ref_xlist, mov_xlist, n_list, 
                 mov_com, mov_to_ref, R, &Eo);
  calculate_rotation_matrixPoint(ref_xlist, mov_xlist, R, U, Eo, &residual, position, distance);
